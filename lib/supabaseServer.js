...
const avgCheck = checks ? Math.round(revenue / checks) : Math.round(average(currentRows, 'avg_check'));
  ...
  return {
    ...
    metrics,
    topDishes: topDishesToday.length ? topDishesToday : (topDishes30Days.length ? topDishes30Days.slice(0, 6) : summary.topDishes),
    topDishes30Days: topDishes30Days.length ? topDishes30Days : summary.topDishes,
    lowDishes: lowDishes.length ? lowDishes : summary.lowDishes,
    waiters: waiters.length ? waiters : summary.waiters,
    waiters30Days: waiters30Days.length ? waiters30Days : summary.waiters,
