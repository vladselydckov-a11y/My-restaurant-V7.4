...
async function getChannelRows(startDate) {
  ...
  const rows = await supabaseFetch(`/rest/v1/channel_sales?select=*${startFilter}&order=business_date.desc&limit=2000`).catch(() => []);
  return Array.isArray(rows) ? rows : [];
}
...
export async function getSupabaseSummary({ restaurantId = 'all' } = {}) {
  ...
  const currentChannelRows = channelRows30.filter((row) => row.business_date === currentDate);
const previousChannelRows = channelRows30.filter((row) => row.business_date === previousDate);

const revenue = sum(currentRows, 'revenue');
const prevRevenue = sum(previousRows, 'revenue');

const checksFromChannels = sum(currentChannelRows, 'checks_count');
const prevChecksFromChannels = sum(previousChannelRows, 'checks_count');
const guestsFromChannels = sum(currentChannelRows, 'guests_count');
const discountsFromChannels = sum(currentChannelRows, 'discount_sum');
const prevDiscountsFromChannels = sum(previousChannelRows, 'discount_sum');

const checks = checksFromChannels || sum(currentRows, 'checks_count');
const prevChecks = prevChecksFromChannels || sum(previousRows, 'checks_count');
const guests = guestsFromChannels || sum(currentRows, 'guests_count');
const discounts = discountsFromChannels || sum(currentRows, 'discount_sum');
...
const prevAvgCheck = prevChecks ? Math.round(prevRevenue / prevChecks) : Math.round(average(previousRows, 'avg_check'));
  ...
  const metrics = [
    ...
    metric('Гости', 'guests', guests, 'из channel_sales', 'good', (value) => String(Math.round(value))),
